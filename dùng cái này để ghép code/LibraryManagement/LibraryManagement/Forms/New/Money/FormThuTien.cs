...
{
    public partial class FormThuTien1 : Form
    {
        ...
        private void nButton1_Click(object sender, EventArgs e)
        {
            ...
            btnLuu.Enabled = false;
                ...
                else
                {
                    ...
                    float k = float.Parse(st) - float.Parse(st2);
                    if (k < 0)
                    {
                    ...
                    else
                    {
                        ...
                        command = connection.CreateCommand();
                        command.CommandText = "insert into PhieuThuTien (MaDocGia,NgThu,SoTienThu,ConLai) values ('" + cbMaDocGia.Text + "','" + formattedStrings[6] + "','" + float.Parse(st2) + "','" + k + "')";
                        command.ExecuteNonQuery();
                        MessageBox.Show("Thanh toán thành công");
                        loadPhieuThu();
                        command = connection.CreateCommand();
                        command.CommandText = "update docgia set TongNo-='" + float.Parse(st2) + "' where madocgia='" + cbMaDocGia.Text + "'";
                        command.ExecuteNonQuery();
        ...
        void loadPhieuThu()
        {
            ...
            //dgvDLPhieuThuTienPhat.Columns[3].DefaultCellStyle.Format = "#,0 VNĐ";
            txbMaPhieuthu.Text = dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount-1].Cells[0].Value.ToString();
            cbMaDocGia.Text= dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount-1].Cells[1].Value.ToString();
            ...
            txbHoTen.Text = table100.Rows[0].ItemArray[0].ToString();
            dtpNgayThu.Value = Convert.ToDateTime(dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount - 1].Cells[2].Value.ToString());
            txbConLai.Text = string.Format("{0:#,0 VNĐ}", float.Parse(dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount - 1].Cells[4].Value.ToString()));
            txbTienThu.Text=string.Format("{0:#,0 VNĐ}", float.Parse(dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount - 1].Cells[3].Value.ToString()));
            txbTongNo.Text = string.Format("{0:#,0 VNĐ}", (float.Parse(Convert.ToString(dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount - 1].Cells[3].Value)) + float.Parse(Convert.ToString(dgvDLPhieuThuTienPhat.Rows[dgvDLPhieuThuTienPhat.RowCount - 1].Cells[4].Value))));
           dgvDLPhieuThuTienPhat.Columns[3].DefaultCellStyle.Format = "#,0 VNĐ";
            dgvDLPhieuThuTienPhat.Columns[4].DefaultCellStyle.Format = "#,0 VNĐ";
            //Gia2.Text = txbTienThu.Text + "VNĐ";
            //Gia3.Text = txbConLai.Text + "VNĐ";
            //Gia1.Text = txbTongNo.Text + "VNĐ";
        ...
        private void comboBox3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                st = float.Parse(table3.Rows[0].ItemArray[0].ToString()).ToString();
                txbTongNo.Text = string.Format("{0:#,0 VNĐ}", float.Parse(table3.Rows[0].ItemArray[0].ToString()));
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex.ToString() != "-1")
            {
                ...
                txbTienThu.Text = float.Parse(Convert.ToString(row.Cells["Số Tiền Thu"].Value)).ToString();
                try
                {
                    cu = cbMaDocGia.Text;
                ...
                txbConLai.Text = string.Format("{0:#,0 VNĐ}", float.Parse(Convert.ToString(row.Cells["Số Tiền Còn nợ"].Value)));
                txbTienThu.Text = string.Format("{0:#,0 VNĐ}", float.Parse(Convert.ToString(row.Cells["Số Tiền Thu"].Value)));
                txbTongNo.Text = string.Format("{0:#,0 VNĐ}", (float.Parse(Convert.ToString(row.Cells["Số Tiền Còn nợ"].Value))) + float.Parse(Convert.ToString(Convert.ToString(row.Cells["Số Tiền Thu"].Value))));
        ...
        private void txbTienThu_Leave(object sender, EventArgs e)
        {
            if (txbTienThu.Text == "")
                txbConLai.Text = "";
            ...
            if (txbTienThu.Text != "" && txbTongNo.Text != "" && txbTienThu.Text != ".")
            {
                ...
                if (float.Parse(st) < float.Parse(txbTienThu.Text))
                {
                    MessageBox.Show("Mời bạn nhập lại, số tiền thu không được lớn hơn số tiền nợ của độc giả");
                    txbTienThu.Text = "";
                }
                ...
                {
                    txbConLai.Text = (float.Parse(st) - float.Parse(txbTienThu.Text)).ToString();
                    st1 = txbConLai.Text;
                    txbConLai.Text = string.Format("{0:#,0 VNĐ}", (float.Parse(st) - float.Parse(txbTienThu.Text)));
                    st2 = txbTienThu.Text;
                    txbTienThu.Text = string.Format("{0:#,0 VNĐ}", float.Parse(txbTienThu.Text));
                }
