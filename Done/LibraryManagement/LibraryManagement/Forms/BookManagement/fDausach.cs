        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            if (ck == 0)
            {
                xuly = 0;
                {
                    ...
                    if (lbMaTL.Text == "")
                    {
                        errMaTL.SetError(lbMaTL, "Vui lòng chọn Mã Thể Loại Sách");
                    }
                    else
                    {
                        errMaTL.Clear();
                ...
                if (txb_TenDauSach.Text.Length > 0 && lbMaTL.Text.Length > 0 && cbTenTL.Text.Length > 0 && lbxTacGia.Items.Count > 0)
                {
                    string query = null;
                    if (xuly == 0)
                    {
