...
namespace TraCuuSach
{
    public partial class FormDanhSachPM : Form
    {
       ...
       (
        ...
        private void FormXemPhieuMuonSach_Load(object sender, EventArgs e)
        {
        ...
        private void UpdateData()
        {
            if (optUpdate != -1)
            {
                ...
                else if (optUpdate == 2)
                {
                    ...
                    {
                        queryUpdateCmd = $@"UPDATE CUONSACH
                        SET TinhTrang = 0
                        ...
                        cmd.ExecuteNonQuery();
                        conn.Close();
                        MessageBox.Show($"Xóa phiếu mượn sách {id} thành công'!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
        ...
        {
            var value = (sender as DateTimePicker).Value;
            if (value != DateTime.ParseExact(borrowSlips[index].returnDate, "dd/MM/yyyy", null) && dateDropdown)
                if (value < DateTime.ParseExact(borrowSlips[index].borrowDate, "dd/MM/yyyy", null))
                {
                    MessageBox.Show("Ngày mượn không thể trễ hơn hạn trả", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    (sender as DateTimePicker).Value = DateTime.ParseExact(borrowSlips[index].returnDate, "dd/MM/yyyy", null);
                    dataChanged = false;
                }
                else
                    dataChanged = true;
        ...
        //private void btnbtnRefresh_Click(object sender, EventArgs e)
        //{
        //    lbSlipId.Text = "";
        //    lbReaderId.Text = "";
        //    lbReaderName.Text = "";
        ...
        {
            new FormCTPM(dtgv.Rows[index].Cells[1].Value.ToString()).ShowDialog();
        }
